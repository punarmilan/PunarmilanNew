...
import { fetchReceivedPhotoRequests } from '../../../Slice/MatchSlice';
...
const PendingRequests = ({ filters, sortBy, filterBy }) => {
    const dispatch = useDispatch();
    const { photoRequestsReceived, loading } = useSelector((state) => state.match);

    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 10;

    useEffect(() => {
        dispatch(fetchReceivedPhotoRequests());
    }, [dispatch]);

    const normalizeRequest = (req) => {
        const profile = req.senderProfile; // Received request, so show sender
        ...
        return {
            ...
            requestType: req.requestType === 'PHOTO' ? 'Photo Request' : 'Request',
            ...
            requestMessage: req.requestType === 'PHOTO' ? 'They requested you to add Photo on' : 'They sent a request on',
            ...
            avatar: '👤',
            img: profile.profilePhotoUrl || "https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=400&h=400&fit=crop",
        };
    };

    const pendingRequests = photoRequestsReceived.map(normalizeRequest).filter(r => r !== null);

    // Filter profiles based on selected filters
    const getFilteredProfiles = () => {
        let processed = [...pendingRequests];
        ...
        // 2. Apply Granular Profile filters (Premium/Verified)
        if (filterBy !== 'all') {
            processed = processed.filter(profile => {
                switch (filterBy) {
        ...
        <div className="animate-fadeIn">
            {currentItems.length > 0 ? (
                ...
                <EmptyState
                    ...
                    submessage="When you receive new requests, they will appear here"
                />
            )}
        </div>
    );
