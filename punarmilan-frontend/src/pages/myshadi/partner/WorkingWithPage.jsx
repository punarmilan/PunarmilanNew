...
const WorkingWithPage = () => {
    ...
    useEffect(() => {
        if (!preferences) {
            ...
            const currentVal = preferences.workingWith || 'Open to All';
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 flex items-center justify-center p-2 sm:p-4 md:p-6">
            <div className="w-full max-w-3xl bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden animate-fadeIn">
                <div className="bg-gradient-to-r from-rose-500 to-rose-600 p-4 sm:p-6 md:p-8">
                    ...
                    <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">Working With</h1>
                    <p className="text-blue-100 text-xs sm:text-sm md:text-base mt-2">Select your preferred work sector</p>
                </div>
                <div className="p-4 sm:p-6 md:p-10">
                    ...
                    <h3 className="text-base sm:text-lg font-bold text-gray-800 mb-4">All Preferences</h3>
                    <div className="space-y-3">
                        {options.map(o => (
                            <label key={o} className="flex items-center gap-3 cursor-pointer group p-4 rounded-xl hover:bg-blue-50 transition-colors border-2 border-transparent hover:border-blue-200">
                                <input type="radio" checked={selected === o} onChange={() => setSelected(o)} className="w-6 h-6 cursor-pointer" />
                                <span className="text-sm sm:text-base font-medium text-gray-700 group-hover:text-gray-900">{o}</span>
                            </label>
                        ))}
                    </div>
